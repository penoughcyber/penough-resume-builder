...
/**
 * Renders a bullet point that may contain HTML formatting.
 * Sanitizes the HTML to only allow safe formatting tags.
 */
function renderBullet(bullet) {
  // If bullet contains HTML tags, sanitize and render as HTML
  if (/<[^>]+>/.test(bullet)) {
    return sanitizeHtml(bullet);
  }
...
/**
 * Checks if content contains bullet type marker (data-bullet attribute).
 */
function getBulletType(content) {
  if (!content) return null;
  const hollowMatch = content.match(/data-bullet=["']hollow["']/i);
  if (hollowMatch) return 'hollow';
  const filledMatch = content.match(/data-bullet=["']filled["']/i);
  if (filledMatch) return 'filled';
  return null;
}

/**
 * Strips the data-bullet wrapper span from content.
 */
function stripBulletWrapper(content) {
  if (!content) return '';
  return content.replace(/<span data-bullet=["'][^"']+["']>([\s\S]*?)<\/span>/gi, '$1');
}

/**
 * Renders experience bullets - as plain paragraphs by default,
 * or as a bulleted list if bullets have bullet type markers.
 */
function renderExperienceBullets(bullets) {
  ...
  if (hasBulletMarkers) {
    // Determine the bullet class (use first found type)
    const bulletType = bulletTypes.find(t => t !== null) || 'filled';
    const bulletClass = bulletType === 'hollow' ? 'bullet-hollow' : 'bullet-filled';
    
    // Render as bulleted list with appropriate class
    return `<ul class="bullets ${bulletClass}">${bullets.map((b) => {
      const content = stripBulletWrapper(b);
      return `<li>${renderBullet(content)}</li>`;
    }).join('')}</ul>`;
  } else {
    // Render as plain paragraphs
    return `<div class="exp-paragraphs">${bullets.map((b) => `<p class="exp-paragraph">${renderBullet(b)}</p>`).join('')}</div>`;
  }
}
 ...
 * - Output is safe (all other content is HTML-escaped)
 */
function renderBoldHtml(text = '') {
  const parts = String(text).split(/(\*\*[^*]+\*\*)/g); // keep the **bold** tokens
  return parts
    .map((part) => {
      const m = part.match(/^\*\*([^*]+)\*\*$/);
...
function normalizeInterestsLines(text = '') {
  ...
  const experiences = (data?.experiences || []).filter((e) => !e.hidden)
    .map(
      ...
      <article class="item-row">
        ...
        ${renderExperienceBullets(e.bullets)}
  ...
  const projects = (data?.projects || []).filter((p) => !p.hidden)
    .map(
      ...
      <article class="item-row">
        ...
        ${renderExperienceBullets(p.bullets)}
      </article>`
    )
    .join('');
  ...
  const html = `
    <div class="cv-page">
      ...
      <main>
        ...
        ${section(
          st.interests || 'Technical Proficiencies',
          data?.interests ? `<div class="cv-paragraph">${interestsHtml}</div>` : ''
        )}
